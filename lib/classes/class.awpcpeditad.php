...
class BoostEditAd {
	...
	public function awpcpui_editformscreen() {
		return self::awpcpui_process_editad();
	}
	...
	public function awpcpui_process_editad() {
		...
		if ( $action == 'editad' ) {
			...
			$output .= self::load_ad_edit_form( $action = 'editad', $awpcppagename, $editemail = '', $adaccesskey = '', $message = '' );
		}
		return $output;
	}

	public function load_ad_edit_form( $action, $awpcppagename, $usereditemail, $adaccesskey, $message ) {
		if ( is_user_logged_in() ) {
			$where    = 'disabled=0 and user_id=' . get_current_user_id();
			$output  .= DisplayAds::awpcp_display_ads( $where, $byl = '', $hidepager = '', $grouporderby, $adorcat = 'ad', $before = '', $editing = true );
		} else {
			$output = "<p>You must be logged in to edit the ads you have posted.</p>";
		}
		return $output;
	}
